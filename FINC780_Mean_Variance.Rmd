...
#  Create finer bins (smaller width) 
seqmret <- seq(min(Results$PortMean),
               max(Results$PortMean),
               length.out = 200)   # instead of 0.001 steps
...
# Smooth the frontier 
smooth_fit <- loess(retn_optim ~ sig_optim, data = optim_frontier, span = 0.2)
...
ggplot() +
  ...
  # SMOOTH CURVE instead of jagged line
  ...
  labs(
    ...
    x = "Portfolio Sigma (Monthly Std Dev)",
    y = "Portfolio Mean (Monthly Return)",
    caption = "Gold diamond = Optimal Portfolio | Green triangle = Min Variance Portfolio"
  ) +
  theme_minimal(base_size = 12) +
